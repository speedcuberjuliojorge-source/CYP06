...
void	Diccionario(char* szNombre, char szPalabras[][TAMTOKEN], int iEstadisticas[], int& iNumElementos)
{
	...
	if (fp != NULL)
	{
		...
		do
		{
			...
			while (!feof(fp))
			{
				...
				{
					//i el caracter es unico e invalido, no guardar
					int banFor = false;
					...
					//si no es caracter invalido individual dejar listo para guardar/comparar
					if (!banFor)
					{
						buffer[pos] = '\0';
	...
	//OBTENER CUANTAS PALABRAS TIENE EL DICCIONARIO
	int frecuencia = 0;
	for (int i = 0; i < a; i++)
	{
		if (strcmp(szPalabras[i], " ") != 0)
